#!/usr/bin/env python
''' Author  : Huy Nguyen
    Program : Relevant Operon PipeLine
                This is basically a program that calls several other program, also provide a comparison
                of old pipeline and new pipeline
    Start   : 08/01/2019
    End     : 09/1/2019
'''
...
def parser_code():
    ...
    parser.add_argument("--gene_blocks","-b", help="The E_Coli/gene_block_names_and_genes.txt file, this file stores the operon name and its set of genes") 
...
#    
    ...
    #    ### generate group.txt file to color taxa based on class
        ...
        cmd9       = './convert.py -i {} -o {} -a {}'.format(result,new_result,"N")
        os.system(cmd9)
        print ('cmd9, 119:',cmd9)   
        ## visulaize the gene block 
        visualization = parent_dir+'/visualization_naive/'
        try:
        ...
        #    ### filtering the gene blocks so that we have the most optimal gene blocks given the blast parse directory, ouput to optimized_gene_block
        optimized_gene_block = parent_dir+'optimized_gene_block/'
        cmd5 ='./findRelevantOperon.py -i {} -o {}'.format(blast_parse,optimized_gene_block)
        os.system(cmd5)
        print ('cmd5:',cmd5)
    #
    #    ### create newick tree file. output in tree directory
    ...
    #    ### generate group.txt file to color taxa based on class
        ...
        ### convert the file in directory result and store it in new_result
        new_result = parent_dir+'new_result/'
        cmd9       = './convert.py -i {} -o {} -a {}'.format(result,new_result, "Y")
        os.system(cmd9)
        print ('cmd9:',cmd9)   
        ## visulaize the gene block 
