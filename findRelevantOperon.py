...
# Copyright(C) 2015 David Ream and Huy Nguyen
...
def parser_code():
    ...
    parser.add_argument("-o", "--outfolder", dest="outfolder", metavar="DIRECTORY", default='./greedy_gene_block/',
...
#        e         = item.e_val()
#        percent_identity = item.percent_ident()
#        bitScore         = item.bits_score()
#        if e>e_val:
#            continue
#        geneName = item.blast_annotation()
#        if accession not in d:
#            d[accession]={geneName:[item]}
#        else:
#            if geneName not in d[accession]:
#                d[accession][geneName]=[item]
#            else:
#                # our current best one
#                current = d[accession][geneName][0]
#                if percent_identity>current.percent_ident():
#                    if bitScore>10+current.bits_score():
#                        temp = [item]
#                        for nextCandidate in d[accession][geneName]:
#                            if abs(nextCandidate.percent_ident()-percent_identity)<1 and abs(bitScore-nextCandidate.bits_score())<10:
#                                temp.append(nextCandidate)
#                        d[accession][geneName]=temp
#                        continue
#                if abs(percent_identity>current.percent_ident())<1 and abs(bitScore-current.bits_score())<10:
#                        d[accession][geneName].append(item)
#                        continue
#    for accession in d:
#        for gene in d[accession]:
#            for hmlog in d[accession][gene]:
#                result.append(hmlog)
...
def main():
    ...
    for fname in file_list:
        ...
        for org in sorted(neighborhood_dict.keys()):
            grouping_list = sorted(list(itertools.chain(*neighborhood_dict[org])), key=lambda x: x.start())
